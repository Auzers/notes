...
大概意思就是：每一个神经元的输出作为下一个神经元的输入，最后一个神经元的输出就是预测结果.每一个神经元都有自己的 w 和 b, 通过迭代训练调整 w 和 b 使成本最小，如果神经网络很复杂，那么 w 和 b 的变化往往是人类不能理解的。
...
概括：K-Means 是一种用**聚类**的算法，将数据集划分为 K 个簇，使同簇数据点更相似，不同簇的数据点差异更大。
