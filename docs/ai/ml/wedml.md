...
**熵 = 混乱程度。** 熵越高，数据越"不纯"；熵为 0，表示数据完全纯净（只有一类）。

直观感受——一袋球：

| 袋子里 | 熵 |
|--------|-----|
| 10 个全是红球 | **0** — 完全确定，毫无混乱 |
| 5 红 5 蓝 | **1** — 最混乱，完全不确定 |
...
> 为什么用 log₂？因为信息论里信息量用"比特"来衡量——回答一个是/否问题恰好消除 1 比特的不确定性。
...
$$
H = -0.643 \times (-0.637) - 0.357 \times (-1.485)
$$
...
根节点熵 = **0.940**。这就是当前的"混乱度"。

**目标：选一个特征切一刀，让熵下降最多。下降的量叫「信息增益」。**

$$
\text{信息增益} = \text{切之前的熵} - \text{切完后的熵（加权平均）}
$$
...
$$
H_{\text{晴}} = -\frac{2}{5}\log_2\frac{2}{5} - \frac{3}{5}\log_2\frac{3}{5}
$$
...
$$
H_{\text{阴}} = -\frac{4}{4}\log_2\frac{4}{4} - 0 = 0 \quad \text{← 完美纯！}
$$

$$
H_{\text{雨}} = -\frac{3}{5}\log_2\frac{3}{5} - \frac{2}{5}\log_2\frac{2}{5} = 0.971
$$

**加权平均（切完后的熵）：**

$$
H_{\text{切后}} = \frac{5}{14} \times 0.971 + \frac{4}{14} \times 0 + \frac{5}{14} \times 0.971
$$
...
$$
\text{Gain}_{\text{天气}} = 0.940 - 0.694 = 0.246
$$

> 解释：用"天气"切一刀，消除了 0.246 比特的不确定性。
...
$$
H_{\text{高}} = -\frac{3}{7}\log_2\frac{3}{7} - \frac{4}{7}\log_2\frac{4}{7} = 0.985
$$

$$
H_{\text{正常}} = -\frac{6}{7}\log_2\frac{6}{7} - \frac{1}{7}\log_2\frac{1}{7} = 0.592
$$

$$
H_{\text{切后}} = \frac{7}{14} \times 0.985 + \frac{7}{14} \times 0.592 = 0.789
$$

$$
\text{Gain}_{\text{湿度}} = 0.940 - 0.789 = 0.151
$$
...
$$
H_{\text{有风}} = 1.0 \quad \text{(3:3 正好一半一半)}
$$

$$
H_{\text{无风}} = -\frac{6}{8}\log_2\frac{6}{8} - \frac{2}{8}\log_2\frac{2}{8} = 0.811
$$

$$
H_{\text{切后}} = \frac{6}{14} \times 1.0 + \frac{8}{14} \times 0.811 = 0.892
$$

$$
\text{Gain}_{\text{有风}} = 0.940 - 0.892 = 0.048
$$
...
阴天熵 = 0，已经完全纯了 → 这个分支**停下**，直接标"去打球"。

晴天和雨天还不够纯，继续切。

---

#### 6.1.6 第二轮：处理晴天（H = 0.971）

晴天的 5 个样本（2✅ 3❌），只考虑「湿度」和「有风」两个剩余特征。
...
$$
H_{\text{高}} = 0 \quad H_{\text{正常}} = 0
$$

$$
H_{\text{切后}} = \frac{3}{5} \times 0 + \frac{2}{5} \times 0 = 0
$$
...
$$
H_{\text{无风}} = -\frac{1}{3}\log_2\frac{1}{3} - \frac{2}{3}\log_2\frac{2}{3} = 0.918
$$

$$
H_{\text{切后}} = \frac{2}{5} \times 1.0 + \frac{3}{5} \times 0.918 = 0.951
$$

$$
\text{Gain} = 0.971 - 0.951 = 0.020
$$

> 湿度增益 0.971 vs 有风增益 0.020 → **湿度碾压。**

---

#### 6.1.7 第三轮：处理雨天（H = 0.971）

雨天的 5 个样本（3✅ 2❌），同样试「湿度」：
...
对于当前节点：
  ...
  2. 对每个可用特征：
     ...
     d. 信息增益 = H - 切完后总熵
  3. 选信息增益最大的特征 → 用它切
  4. 对每个子节点，如果没纯（熵 > 0），递归回到第 1 步
```

**预测阶段：**

```
新样本来了 → 按照树的条件一路往下走 → 走到叶子 → 叶子说什么就是什么
```
...
> **信息增益衡量的是"这个特征能帮我消除多少不确定性"。每次挑最能消除不确定性的特征来切，直到所有分支都确定为止。**
